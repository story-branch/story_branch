...
module StoryBranch
  ...
  class Main
    ...
    def finish
      ...
      unless is_current_branch_a_story?
        puts "Your current branch: #{git_current_branch}"
        puts "is not linked to a Pivotal Tracker Story."
        return
      end
    ...
    def is_current_branch_a_story?
      ...
      story_id = name.match(/-(\d+$)/)
      return false unless story_id.length == 2
      stories = pivotal_stories_list.map(&:id)
      stories.include? story_id[1].to_i
    end

    def pivotal_stories_list state=:started
      get_project.stories.all({current_state: state})
    end

    def display_pivotal_stories
      stories = pivotal_stories_list
    ...
    def pivotal_story_branch
      stories = display_pivotal_stories
