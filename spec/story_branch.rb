  ...
  context "Story Branch initialization" do
    ...
    context "no PIVOTAL_API_KEY defined in env" do
      ...
      it "should exit with error message config file found in path is empty" do
        copy_config_files %w(spec/files/.story_branch), %w(true)
        expect{
          sb = StoryBranch.new
        }.to raise_error("Existing .story_branch config file found, but without contents")
        expect(sb).to be_nil
      end
    end
    ...
    it "should be a valid object with api key set based on environment and project_id set based on config file" do
      ENV['PIVOTAL_API_KEY']="ABC123DEF"
      copy_config_files %w(spec/files/.story_branch)
      expect{
        sb = StoryBranch.new
      }.to_not raise_error
      sb = StoryBranch.new
    ...
    it "should be a valid object with api_key and project_id both set based on config files" do
      copy_config_files %w(spec/files/.story_branch spec/files/.story_branch)
      expect{
        sb = StoryBranch.new
      }.to_not raise_error
      sb = StoryBranch.new
