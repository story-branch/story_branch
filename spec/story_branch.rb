...
describe StoryBranch do
  ...
  let(:project_id_error_message_rx) { Regexp.new(/\$PIVOTAL_PROJECT_ID must be set/) }
  let(:api_key_error_message_rx)    { Regexp.new(/\$PIVOTAL_API_KEY must be set/) }
    ...
    context "Story Branch initialization" do
      ...
      context "no PIVOTAL_API_KEY defined in env" do
        it "should exit with error message if no config file is found in the current path" do
          expect{begin StoryBranch::Main.new; rescue SystemExit; end}.to output(project_id_error_message_rx).to_stdout
          expect{begin StoryBranch::Main.new; rescue SystemExit; end}.to output(api_key_error_message_rx).to_stdout
        end
      ...
      context "PIVOTAL_API_KEY is set" do
        it "should exit with error message if no PIVOTAL_PROJECT_ID is defined in enviroment and no config file is found in the current path" do
          ...
          expect{begin StoryBranch::Main.new; rescue SystemExit; end}.to output(project_id_error_message_rx).to_stdout
          expect{begin StoryBranch::Main.new; rescue SystemExit; end}.not_to output(api_key_error_message_rx).to_stdout
        end
      ...
      it "should be a valid object with api_key and project_id both set based on environmental variables" do
        ...
        expect{begin StoryBranch::Main.new; rescue SystemExit; end}.not_to output(project_id_error_message_rx).to_stdout
        expect{begin StoryBranch::Main.new; rescue SystemExit; end}.not_to output(api_key_error_message_rx).to_stdout
      end
      ...
      it "should be a valid object with api key set based on environment and project_id set based on config file" do
        ...
        expect{begin StoryBranch::Main.new; rescue SystemExit; end}.to_not output(project_id_error_message_rx).to_stdout
        expect{begin StoryBranch::Main.new; rescue SystemExit; end}.to_not output(api_key_error_message_rx).to_stdout
      end
